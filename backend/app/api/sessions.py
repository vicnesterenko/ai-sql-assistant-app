...
async def create_session_endpoint(payload: CreateSessionRequest, user: CurrentUser = Depends(get_current_user)) -> SessionResponse:
...
async def post_message(session_id: str, payload: ChatMessageRequest, user: CurrentUser = Depends(get_current_user)):
    ...
    await save_message(session_id, payload.thread_id, 'assistant', final_state.final_response.message, final_state.final_response)
