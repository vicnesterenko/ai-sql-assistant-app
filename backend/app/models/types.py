...
class Intent(BaseModel):
    question: str = Field(description='Resolved natural-language analytics question after considering conversation context.')
    is_follow_up: bool = Field(default=False, description='True when the current user message depends on previous SQL or previous question.')
    assumptions: list[str] = Field(default_factory=list, description='Reasonable defaults made for ambiguous user input and surfaced to the user.')
    referenced_previous_sql: str | None = Field(default=None, description='Previous SQL used to resolve this follow-up, if any.')


class ValidationResult(BaseModel):
    is_valid: bool = Field(description='Whether the generated SQL passed static validation and is safe to continue.')
    errors: list[str] = Field(default_factory=list, description='Validation errors that must be fixed before execution or approval.')
    warnings: list[str] = Field(default_factory=list, description='Non-blocking validation warnings, for example missing WHERE on large table.')
    referenced_tables: list[str] = Field(default_factory=list, description='Tables referenced by the SQL statement.')
    referenced_columns: list[str] = Field(default_factory=list, description='Columns referenced by the SQL statement in table.column or column format.')
...
class ApprovalDecision(BaseModel):
    ...
    approver_email: str | None = Field(default=None, description='Email of the human approver who resolved the request.')
    modified_sql: str | None = Field(default=None, description='Approver-edited SQL. If omitted, the generated SQL is approved as-is.')
...
class ExecutionResult(BaseModel):
    ...
    rows: list[dict[str, Any]] = Field(default_factory=list, description='Truncated row payload returned to UI. Not stored in LLM memory.')
    row_count: int = Field(default=0, description='Number of rows returned after truncation.')
    truncated: bool = Field(default=False, description='True when MAX_RESULT_ROWS was exceeded and response rows were truncated.')
    duration_ms: int = Field(default=0, description='Actual database execution duration in milliseconds.')
    error_message: str | None = Field(default=None, description='Database error or timeout message, if execution failed.')
...
class SQLAssistantState(BaseModel):
    ...
    messages: list[dict[str, str]] = Field(default_factory=list, description='Conversation message history containing roles and sanitized content.')
    ...
    approval_decision: ApprovalDecision | None = Field(default=None, description='Human approval decision when graph resumes.')
    approved_sql: str | None = Field(default=None, description='Final SQL after approval, possibly modified by approver.')
    ...
    final_response: AssistantResponse | None = Field(default=None, description='Final API response returned to frontend.')
